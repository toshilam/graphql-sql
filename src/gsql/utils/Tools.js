...
(function(){
	...
	Tools.loadNodeClasses = function(classes)
	{
		...
		for (var i = 0; i < numClass; i++) 
		{
			...
			//gsql.Tracer.echo('Tools : loadNodeClasses : loading : ' + path, this, gsql.Tracer.TYPE_DEBUG);
	...
	{
		if(gsql.Tools.hasValue(inData) && Tools.isDateString(inData, inData.split(' ').length == 2))
		{
	...
	Tools.decodeUriComponent = function(data)
	{
		return _.isString(data) ? decodeUriComponent(data) : _.each(data, (v,i,o)=>{
			o[i] = _.isObject(o[i]) || _.isArray(o[i]) ? Tools.decodeUriComponent(o[i]) : _.isString(o[i]) ? decodeUriComponent(o[i]) : v;
		});
	}
